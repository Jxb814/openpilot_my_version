...
from openpilot.tools.lib.logreader import LogReader, ReadMode, _LogFileReader, save_log
...
def juggle_route(route_or_segment_name, can, layout, dbc, should_migrate):
  ...
  out_dir = "/home/hatci/openpilot/tools/Downloads/"+route_or_segment_name
  os.makedirs(out_dir, exist_ok=True)

  # iterate over each segment
  for i, seg_file in enumerate(lr.logreader_identifiers):
    # load the raw segment
    _lr = _LogFileReader(seg_file)
    # save this segment
    out_path = os.path.join(out_dir, f"{i}_processed_rlog.zst")
    save_log(out_path, _lr, compress=True)
    print(f"Saved segment {i} → {out_path}")
